...
const AdminAnalytics = React.memo(() => {
  const theme = useTheme()
  ...
  // All hooks are now called after the early return
  // But we need to make sure all useMemo hooks are called in the same order every time
  // The issue was that useMemo was being called conditionally based on analytics data
  ...
  // Prepare chart data - always define these even if data is empty
  const userGrowthData = userData.userGrowthTrend || [
  ...
  const postActivityData = postData.postActivityTrend || [
  ...
  const mediaTypeChartData = Object.entries(mediaTypeData).map(([key, value]) => ({
    type: key,
    count: value,
  }))
    ...
    <Box>
      ...
      <Grid container spacing={3}>
        <Grid size={{ xs: 12, md: 6 }}>
          <ChartSection title="User Growth">
